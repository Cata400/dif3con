...
from datasets.datasets import ImagesPathDataset, make_dataset
...
def get_activations(
    ...
    dataset = ImagesPathDataset(files, transforms=TF.ToTensor())
    ...
    for batch in track(dataloader, total=len(dataloader), console=Console(stderr=True)):
...
def get_activations_for_batch(batch, model):
    model.eval()
...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=1, num_samples=-1):
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1, num_samples=-1):
    ...
    m1, s1 = compute_statistics_of_path(
        paths[0], model, batch_size, dims, device, num_workers, num_samples
    )
    m2, s2 = compute_statistics_of_path(
        paths[1], model, batch_size, dims, device, num_workers, num_samples
    )
...
def main():  
    ...
    fid_value = calculate_fid_given_paths(
        args.path, args.batch_size, device, args.dims, num_workers, args.num_samples
    )
    print("FID: ", fid_value)
